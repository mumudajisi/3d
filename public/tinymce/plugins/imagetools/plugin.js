...
(function () {
  ...
  var applyRotate = function (image, type, angle) {
    ...
    var newWidth = ceilWithPrecision(
      Math.abs(width * cos) + Math.abs(height * sin)
    );
    var newHeight = ceilWithPrecision(
  ...
  var isLocalImage = function (editor, img) {
    ...
    return (
      url.indexOf("data:") === 0 ||
      url.indexOf("blob:") === 0 ||
      new global$1(url).host === editor.documentBaseURI.host
    );
  };
  var isCorsImage = function (editor, img) {
    return (
      global$4.inArray(getCorsHosts(editor), new global$1(img.src).host) !== -1
    );
  };
